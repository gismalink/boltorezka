...
- Node.js: >= 22.12.0 (рекомендуется Node 22 LTS; версия зафиксирована в `.nvmrc`).
...
- `SMOKE_API_URL=https://test.datowave.com npm run smoke:sso` — SSO redirect/mode smoke (`scripts/smoke/smoke-sso-redirect.mjs`)
- `SMOKE_API_URL=https://test.datowave.com SMOKE_BEARER_TOKEN=<jwt> npm run smoke:realtime` — WS protocol smoke (`nack/ack/idempotency`) через `ws-ticket` (`scripts/smoke/smoke-realtime.mjs`)
- `SMOKE_CALL_SIGNAL=1 SMOKE_API_URL=https://test.datowave.com SMOKE_BEARER_TOKEN=<jwt1> SMOKE_BEARER_TOKEN_SECOND=<jwt2> npm run smoke:realtime` — расширенный WS smoke + relay проверки `call.offer`, `call.reject` и `call.hangup` с гарантированно разными пользователями
...
- `SMOKE_API_URL=https://test.datowave.com SMOKE_API=1 SMOKE_SSO=1 npm run check` — единый verify + API + SSO smoke
- `SMOKE_API_URL=https://test.datowave.com SMOKE_API=1 SMOKE_SSO=1 SMOKE_REALTIME=1 SMOKE_WS_TICKET=<ticket> npm run check` — полный verify + API + SSO + realtime smoke
...
- Required repository variable: `TEST_SMOKE_API_URL` (optional, default `https://test.datowave.com`).
...
- `set -a; source .deploy/smoke-auth.env; set +a && SMOKE_WEB_BASE_URL=https://test.datowave.com npm run smoke:web:e2e`
...
4. Для локального SSO по умолчанию используется `AUTH_SSO_BASE_URL=http://localhost:3000`.
   Для test/prod окружений переопредели на `https://test.auth.datowave.com` / `https://auth.datowave.com`.
...
- `test.datowave.com` — тестовый контур
- `datowave.com` — продовый контур

Для `test` окружения обязательно:

- `AUTH_MODE=sso`
- `AUTH_SSO_BASE_URL=https://test.auth.datowave.com`
- `ALLOWED_RETURN_HOSTS` включает `test.datowave.com`

Для `prod` окружения:

- `AUTH_MODE=sso`
- `AUTH_SSO_BASE_URL=https://auth.datowave.com`
- `ALLOWED_RETURN_HOSTS` включает `datowave.com`

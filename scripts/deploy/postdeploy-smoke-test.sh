...
SMOKE_CHAT_STORAGE_PUT_OK_DELTA=0
SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA=0
SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS="skip"
...
MULTISERVER_AGE_GATE_STATUS="skip"
MULTISERVER_ROLE_MATRIX_STATUS="skip"
...
write_summary() {
  ...
  printf 'SMOKE_MULTISERVER_AGE_GATE_STATUS=%q\n' "$MULTISERVER_AGE_GATE_STATUS" >>"$SUMMARY_FILE_REL"
  printf 'SMOKE_MULTISERVER_ROLE_MATRIX_STATUS=%q\n' "$MULTISERVER_ROLE_MATRIX_STATUS" >>"$SUMMARY_FILE_REL"
  ...
  printf 'SMOKE_CHAT_STORAGE_PUT_OK_DELTA=%q\n' "$SMOKE_CHAT_STORAGE_PUT_OK_DELTA" >>"$SUMMARY_FILE_REL"
  printf 'SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA=%q\n' "$SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA" >>"$SUMMARY_FILE_REL"
  printf 'SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS=%q\n' "$SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS" >>"$SUMMARY_FILE_REL"
...
mint_smoke_bearer_for_email() {
  ...
  if [[ -z "$email" || -z "${JWT_SECRET_CANDIDATE:-}" ]]; then
    return 0
  fi

  local user_meta user_id user_role session_id bearer auth_payload
  ...
  bearer="$(make_hs256_jwt "$JWT_SECRET_CANDIDATE" "$user_id" "$user_role" "$session_id" "sso")"
  auth_payload="$(printf '{"userId":"%s","authMode":"sso","issuedAt":"%s","rotatedFrom":null}' "$user_id" "$(date -u +%Y-%m-%dT%H:%M:%SZ)")"
  compose exec -T "$REDIS_SERVICE" redis-cli SETEX "auth:session:$session_id" "${SMOKE_AUTH_SESSION_TTL_SEC:-2592000}" "$auth_payload" >/dev/null
  export "$target_var=$bearer"
}
...
read_ws_metric_current_day() {
  local key="$1"
  local day value
  day="$(date -u +%F)"

  value="$(compose exec -T "$REDIS_SERVICE" redis-cli HGET "ws:metrics:$day" "$key" 2>/dev/null | tr -d '\r' | tail -n1 || true)"
...
if [[ -n "${JWT_SECRET_CANDIDATE:-}" ]]; then
  mint_smoke_bearer_for_email "$USER_EMAIL_SECOND" SMOKE_TEST_BEARER_TOKEN_SECOND
  mint_smoke_bearer_for_email "${SMOKE_USER_EMAIL_ADMIN:-}" SMOKE_TEST_BEARER_TOKEN_ADMIN
fi
...
if [[ "${SMOKE_API:-1}" == "0" ]]; then
  ...
  CHAT_STORAGE_PUT_OK_BEFORE="$(read_ws_metric_current_day "chat_storage_put_ok")"
  CHAT_STORAGE_PUT_FAIL_BEFORE="$(read_ws_metric_current_day "chat_storage_put_fail")"
  ...
  if [[ "${SMOKE_CHAT_ORPHAN_CLEANUP:-0}" == "1" ]]; then
    if [[ -z "${SMOKE_TEST_BEARER_TOKEN:-}" ]]; then
      echo "[postdeploy-smoke] smoke:chat:orphan-cleanup skipped (no bearer token)"
      CHAT_ORPHAN_CLEANUP_STATUS="skip"
    else
      echo "[postdeploy-smoke] smoke:chat:orphan-cleanup"
      SMOKE_API_URL="$BASE_URL" SMOKE_TEST_BEARER_TOKEN="${SMOKE_TEST_BEARER_TOKEN:-}" npm run smoke:chat:orphan-cleanup
      CHAT_ORPHAN_CLEANUP_STATUS="pass"
    fi
  else
  ...
  CHAT_STORAGE_PUT_OK_AFTER="$(read_ws_metric_current_day "chat_storage_put_ok")"
  CHAT_STORAGE_PUT_FAIL_AFTER="$(read_ws_metric_current_day "chat_storage_put_fail")"
  SMOKE_CHAT_STORAGE_PUT_OK_DELTA=$((CHAT_STORAGE_PUT_OK_AFTER - CHAT_STORAGE_PUT_OK_BEFORE))
  SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA=$((CHAT_STORAGE_PUT_FAIL_AFTER - CHAT_STORAGE_PUT_FAIL_BEFORE))

  if [[ "${SMOKE_CHAT_STORAGE_METRICS:-1}" == "1" ]]; then
    SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS="pass"
    echo "[postdeploy-smoke] chat storage write metrics: ok_delta=${SMOKE_CHAT_STORAGE_PUT_OK_DELTA} fail_delta=${SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA}"

    if (( SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA > ${SMOKE_CHAT_STORAGE_PUT_FAIL_THRESHOLD:-0} )); then
      SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS="fail"
      echo "[postdeploy-smoke] chat storage write failure threshold exceeded: ${SMOKE_CHAT_STORAGE_PUT_FAIL_DELTA} > ${SMOKE_CHAT_STORAGE_PUT_FAIL_THRESHOLD:-0}" >&2
      if [[ "${SMOKE_CHAT_STORAGE_METRICS_STRICT:-1}" == "1" ]]; then
        exit 1
      fi
      SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS="warn"
      echo "[postdeploy-smoke] continuing because SMOKE_CHAT_STORAGE_METRICS_STRICT=0" >&2
    fi
  else
    SMOKE_CHAT_STORAGE_PUT_FAIL_STATUS="skip"
    echo "[postdeploy-smoke] chat storage metrics gate skipped (SMOKE_CHAT_STORAGE_METRICS=0)"
  fi
fi
...
if [[ "${SMOKE_WEB_NETWORK_REQUESTS_BROWSER:-1}" == "1" ]]; then
  if [[ ! -d "node_modules/playwright" ]]; then
    echo "[postdeploy-smoke] install npm dependencies (playwright missing)"
    npm install --no-audit --no-fund
  fi

  echo "[postdeploy-smoke] smoke:web:network-requests:browser"
  SMOKE_API_URL="$BASE_URL" \
    SMOKE_WEB_BASE_URL="$WEB_BASE_URL" \
    SMOKE_TEST_BEARER_TOKEN="${SMOKE_TEST_BEARER_TOKEN:-}" \
    npm run smoke:web:network-requests:browser
  WEB_NETWORK_REQUESTS_STATUS="pass"
else
...
if [[ "${SMOKE_MULTISERVER:-0}" == "1" ]]; then
  if ! SMOKE_TEST_BEARER_TOKEN="$(mint_fresh_smoke_bearer "smoke:multiserver")"; then
    ...
    echo "[postdeploy-smoke] smoke:multiserver skipped (missing bearer token pair)"
    MULTISERVER_SMOKE_STATUS="skip"
  else
    echo "[postdeploy-smoke] smoke:multiserver"
    SMOKE_API_URL="$BASE_URL" \
      SMOKE_TEST_BEARER_TOKEN="${SMOKE_TEST_BEARER_TOKEN:-}" \
      SMOKE_TEST_BEARER_TOKEN_SECOND="${SMOKE_TEST_BEARER_TOKEN_SECOND:-}" \
      SMOKE_TEST_BEARER_TOKEN_ADMIN="${SMOKE_TEST_BEARER_TOKEN_ADMIN:-}" \
      npm run smoke:multiserver
    MULTISERVER_SMOKE_STATUS="pass"

    if [[ "${SMOKE_MULTISERVER_AGE_GATE:-1}" == "1" ]]; then
      echo "[postdeploy-smoke] smoke:multiserver:age-gate"
      SMOKE_API_URL="$BASE_URL" \
        SMOKE_ENV_SCOPE="$SMOKE_ENV_SCOPE" \
        SMOKE_USER_EMAIL="$USER_EMAIL" \
        SMOKE_USER_EMAIL_SECOND="$USER_EMAIL_SECOND" \
        npm run smoke:multiserver:age-gate
      MULTISERVER_AGE_GATE_STATUS="pass"
    else
      echo "[postdeploy-smoke] smoke:multiserver:age-gate skipped (SMOKE_MULTISERVER_AGE_GATE=0)"
      MULTISERVER_AGE_GATE_STATUS="skip"
    fi

    if [[ "${SMOKE_MULTISERVER_ROLE_MATRIX:-1}" == "1" ]]; then
      echo "[postdeploy-smoke] smoke:multiserver:role-matrix"
      SMOKE_ENV_SCOPE="$SMOKE_ENV_SCOPE" npm run smoke:multiserver:role-matrix
      MULTISERVER_ROLE_MATRIX_STATUS="pass"
    else
      echo "[postdeploy-smoke] smoke:multiserver:role-matrix skipped (SMOKE_MULTISERVER_ROLE_MATRIX=0)"
      MULTISERVER_ROLE_MATRIX_STATUS="skip"
    fi
  fi
else
  echo "[postdeploy-smoke] smoke:multiserver skipped (SMOKE_MULTISERVER=0)"
  MULTISERVER_SMOKE_STATUS="skip"
  MULTISERVER_AGE_GATE_STATUS="skip"
  MULTISERVER_ROLE_MATRIX_STATUS="skip"
fi
...
if [[ "${SMOKE_REALTIME_MEDIA:-0}" == "1" ]]; then
  ...
  if [[ -n "${TURN_USERNAME:-}" && -n "${TURN_PASSWORD:-}" ]]; then
    media_turn_domain="${TURN_CERT_DOMAIN:-turns.datowave.com}"

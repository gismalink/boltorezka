...
POSTGRES_SERVICE="${SMOKE_AUTH_POSTGRES_SERVICE:-datowave-db-test}"
API_SERVICE="${SMOKE_AUTH_API_SERVICE:-datowave-api-test}"
...
RUN_AGENT_SEMANTICS_BROWSER="${SMOKE_ALL_RUN_AGENT_SEMANTICS_BROWSER:-1}"
...
if [[ "$RUN_WEB_E2E" == "1" ]]; then
  run_case "smoke:web:e2e" \
    ...
    SMOKE_E2E_AGENT_SEMANTICS_BROWSER="$RUN_AGENT_SEMANTICS_BROWSER" \
    SMOKE_EXPECT_MEDIA_TOPOLOGY="$EXPECT_TOPOLOGY" \
    SMOKE_TIMEOUT_MS="$SMOKE_TIMEOUT_MS" \

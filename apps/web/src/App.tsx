/**
 * App.tsx — корневой оркестратор клиентского приложения.
 *
 * Назначение:
 * - Подключает все runtime-хуки (auth, realtime, voice, rooms, UI и т.д.) и склеивает их пропсами.
 * - Передаёт собранные пропсы в `AppShellLayout` (главный shell).
 * - Сам файл должен оставаться свободным от бизнес-логики.
 *
 * Куда класть новую логику:
 * - Состояние/эффекты — в соответствующий хук в `hooks/`.
 * - Сетевые/realtime-команды — в `services/`.
 * - UI-поведение — в компоненты.
 *
 * НЕ добавляйте сюда: парсинг сообщений, правила транспорта, маршрутизацию.
 */
...
export function App() {
  ...
  const {
    ...
    pendingChatAttachmentFiles, pendingChatAttachmentStateByKey, selectAttachmentFiles, removePendingAttachmentAt, clearPendingAttachment,
  ...
  const roomsPanelRuntimeInput = {
    t, canCreateRooms, canManageAudioQuality,
    roomsTree, roomsTreeLoading, roomsTreeBootstrapPending,
    roomSlug, chatRoomSlug,
    screenShareOwnerByRoomSlug,
    ...
    newCategoryTitle, categoryPopupOpen,
    newRoomTitle, newRoomKind, newRoomCategoryId,
    channelPopupOpen, categorySettingsPopupOpenId,
    editingCategoryTitle, channelSettingsPopupOpenId,
    editingRoomTitle, editingRoomKind, editingRoomCategoryId,
    editingRoomNsfw, editingRoomHidden, editingRoomAudioQualitySetting,
    categoryPopupRef, channelPopupRef,
    setCategoryPopupOpen, setChannelPopupOpen,
    setNewCategoryTitle, setNewRoomTitle,
    setNewRoomKind, setNewRoomCategoryId,
    setEditingCategoryTitle,
    setEditingRoomTitle, setEditingRoomKind, setEditingRoomCategoryId,
    setEditingRoomNsfw, setEditingRoomHidden, setEditingRoomAudioQualitySetting,
    createCategory, createRoom,
    openCreateChannelPopup, openCategorySettingsPopup, openChannelSettingsPopup,
    saveCategorySettings, moveCategory, deleteCategory,
    saveChannelSettings, moveChannel,
    clearChannelMessages, deleteChannel, restoreChannel, deleteChannelPermanent,
    onToggleCategoryCollapsed: toggleCategoryCollapsed,
    onJoinRoom: joinRoom, onOpenRoomChat: openRoomChat,
    onKickRoomMember: kickRoomMember, onMoveRoomMember: moveRoomMember,
    onSaveMemberPreference: saveMemberPreference,
    onApplyLocalMemberVolume: applyLocalMemberVolume,
    onLoadServerMemberProfile: loadServerMemberProfile,
    onLoadServerRoles: loadServerRoles,
    onSetServerMemberCustomRoles: handleSetServerMemberCustomRoles,
    onSetServerMemberHiddenRoomAccess: handleSetServerMemberHiddenRoomAccess
  };

  const serverProfileModalRuntimeInput = {
    user, currentServer,
    canManageUsers, canPromote, canManageServerControlPlane,
    canViewTelemetry, canManageAudioQuality,
    serverMenuTab, serverAudioQuality, serverAudioQualitySaving, serverChatImagePolicy,
    serverVideoEffectType, serverVideoResolution, serverVideoFps,
    serverScreenShareResolution,
    serverVideoPixelFxStrength, serverVideoPixelFxPixelSize, serverVideoPixelFxGridThickness,
    serverVideoAsciiCellSize, serverVideoAsciiContrast, serverVideoAsciiColor,
    normalizedServerVideoWindowMinWidth, normalizedServerVideoWindowMaxWidth,
    adminUsers, adminServers, adminServersLoading,
    selectedAdminServerId, adminServerOverview, adminServerOverviewLoading,
    currentServerId, servers,
    serverMembers, serverMembersLoading,
    lastInviteUrl, eventLog, telemetrySummary,
    callStatus, lastCallPeer, roomVoiceConnected, callEventLog,
    serverVideoPreviewStream,
    setAppMenuOpen, setServerMenuTab,
    promote, demote, setUserBan, setUserAccessState,
    deleteUser, forceDeleteUserNow,
    setSelectedAdminServerId,
    handleToggleAdminServerBlocked, handleDeleteAdminServer,
    handleCreateServerInvite, handleCopyInviteUrl,
    handleServerChange, handleRenameCurrentServer,
    handleLeaveCurrentServer, handleDeleteCurrentServer,
    handleRemoveServerMember, handleBanServerMember, handleUnbanServerMember,
    handleTransferServerOwnership,
    loadServerMemberProfile, loadServerRoles,
    handleCreateServerRole, handleRenameServerRole, handleDeleteServerRole,
    handleSetServerMemberCustomRoles, handleSetServerMemberHiddenRoomAccess,
    loadTelemetrySummary,
    setServerAudioQualityValue,
    setServerVideoEffectType, setServerVideoResolution, setServerVideoFps,
    setServerScreenShareResolution,
  ...
  const chatVideoRuntimeInput = {
    ...
    chatText, pendingChatImageDataUrl, pendingChatAttachmentFiles, pendingChatAttachmentStateByKey,
    ...
    sendMessage, selectAttachmentFiles, removePendingAttachmentAt, retryPendingAttachmentAt, clearPendingAttachment,
    editingMessageId, replyingToMessageId,
    currentRoomSupportsRtc,
    videoWindowsVisible, setVideoWindowsVisible,
    setEditingMessageId, setReplyingToMessageId,
    startEditingMessage, replyToMessage, cancelReply,
    deleteOwnMessage, reportMessage,
    pinnedByMessageId, reactionsByMessageId,
    togglePinMessage, toggleMessageReaction,
    updateTopic, archiveTopic, unarchiveTopic, deleteTopic,
    allowVideoStreaming, cameraEnabled,
    localVideoStream, remoteVideoStreamsByUserId,
    effectiveVoiceCameraEnabledByUserIdInCurrentRoom,
    remoteVideoLabelsByUserId,
    activeScreenShare,
    normalizedServerVideoWindowMinWidth, normalizedServerVideoWindowMaxWidth,
  ...
  const {
    ...
    token,
    roomsPanel: roomsPanelRuntimeInput,
    serverProfileModal: serverProfileModalRuntimeInput,
    chatVideo: chatVideoRuntimeInput
  }));

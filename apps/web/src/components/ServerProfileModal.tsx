/**
 * ServerProfileModal.tsx — крупная модальная панель профиля сервера.
 *
 * Назначение:
 * - Показывает вкладки сервера: участники, роли, приглашения, настройки, баны.
 * - Использует admin/owner-API и локальные хуки; бизнес-логика распределена внутри.
 *
 * ВНИМАНИЕ: файл очень большой (>1500 строк). Рассматривать разбиение на вкладки
 * в отдельные файлы (`./serverProfileModal/*`).
 */
        ...
        <div className="user-settings-content server-scroll-pane grid min-h-0 min-w-0 content-start gap-4 overflow-auto overflow-x-hidden pr-0">
          ...
          {serverMenuTab === "video" && canManageAudioQuality ? (
            <ServerVideoSettingsTab
              ...
              serverVideoWindowMinWidth={serverVideoWindowMinWidth}
              serverVideoWindowMaxWidth={serverVideoWindowMaxWidth}
              serverVideoPreviewStream={serverVideoPreviewStream}
              onSetServerVideoEffectType={onSetServerVideoEffectType}
              onSetServerVideoResolution={onSetServerVideoResolution}
              onSetServerVideoFps={onSetServerVideoFps}
              onSetServerScreenShareResolution={onSetServerScreenShareResolution}
              onSetServerVideoPixelFxStrength={onSetServerVideoPixelFxStrength}
              onSetServerVideoPixelFxPixelSize={onSetServerVideoPixelFxPixelSize}
              onSetServerVideoPixelFxGridThickness={onSetServerVideoPixelFxGridThickness}
              onSetServerVideoAsciiCellSize={onSetServerVideoAsciiCellSize}
              onSetServerVideoAsciiContrast={onSetServerVideoAsciiContrast}
              onSetServerVideoAsciiColor={onSetServerVideoAsciiColor}
              onSetServerVideoWindowMinWidth={onSetServerVideoWindowMinWidth}
              onSetServerVideoWindowMaxWidth={onSetServerVideoWindowMaxWidth}
            />
          ) : null}

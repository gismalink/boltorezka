...
- Прогресс 2026-04-10: добавлен `anchorMessageId` в `GET /v1/topics/:topicId/messages` и подключен в web jump-flow (поиск + mentions) для детерминированной загрузки контекста вокруг target message; параметризация before/after window остаётся pending.

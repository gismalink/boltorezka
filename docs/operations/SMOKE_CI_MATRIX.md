...
| Browser media transport + one-way gate (SFU profile) | included in `deploy:test:sfu` via `SMOKE_REALTIME_MEDIA=1` and `SMOKE_FAIL_ON_ONE_WAY=1` (strict mode opt-in: `SMOKE_REALTIME_MEDIA_STRICT=1`) | Advisory by default |

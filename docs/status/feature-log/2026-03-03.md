...
- Локально: `SMOKE_API_URL=https://test.datowave.com npm run smoke:web:static` — PASS.
- Test deploy/smoke: `TEST_REF=origin/main ALLOW_TEST_FROM_MAIN=1 npm run deploy:test:smoke` — PASS.
- Серверный web e2e (test): `SMOKE_API_URL=https://test.datowave.com SMOKE_E2E_CALL_SIGNAL=0 npm run smoke:web:e2e` — PASS.
...
- `ssh mac-mini 'cd ~/srv/datowave && TEST_REF=a159e87 npm run deploy:test:smoke'`
- `ssh mac-mini 'cd ~/srv/datowave && TEST_REF=origin/main ALLOW_TEST_FROM_MAIN=1 npm run deploy:test:smoke'`

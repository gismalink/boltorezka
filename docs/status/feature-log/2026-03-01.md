...
- `docker compose ps`:
  - `datowave-api-prod` — `Up`,
  - `datowave-db-prod` — `Up (healthy)`,
  - `datowave-redis-prod` — `Up (healthy)`.
- `curl -I https://datowave.com/health` -> `HTTP/2 200`.
- `curl https://datowave.com/health` -> `{"status":"ok","checks":{"api":"ok","db":"ok","redis":"ok"},...}`.
- `curl https://datowave.com/v1/auth/mode` -> `{"mode":"sso","ssoBaseUrl":"https://auth.gismalink.art"}`.

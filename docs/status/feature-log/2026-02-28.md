...
- В web sidebar добавлено сворачивание/разворачивание категорий:
  ...
  - persist состояния в `localStorage` (`datowave_collapsed_category_ids`).
...
- Command: `ssh mac-mini 'cd ~/srv/datowave && TEST_REF=origin/feature/web-header-profile-menu npm run deploy:test:smoke'`.
- Smoke result:
  - `smoke:sso` — PASS,
  - `smoke:realtime` — PASS,
...
- Сид применён в `test` БД на сервере (`datowave-db-test`) через `docker compose exec ... psql`.
...
- Command pattern: `cat /tmp/datowave_seed_chatset.sql | docker compose ... exec -T datowave-db-test psql ...`.
...
- Added persistence for selected audio devices:
  - `datowave_selected_input_id`,
  - `datowave_selected_output_id` (restore on reload).
...
- Quickstart/runbook обновлены на актуальный Datowave test deploy flow.
...
- Test rollout/smoke: `ssh mac-mini 'cd ~/srv/datowave && TEST_REF=origin/feature/web-header-profile-menu npm run deploy:test:smoke'` — PASS.
- Smoke result:
  - `smoke:sso` — PASS,
  - `smoke:realtime` — PASS,
  - `reconnectOk=true`.
...
- Roadmap big-block (web e2e smoke): добавлен единый оркестратор `scripts/smoke-web-e2e.sh` (`smoke:sso` + `smoke:realtime` с `SMOKE_CALL_SIGNAL=1`, `SMOKE_RECONNECT=1`), подключён `npm run smoke:web:e2e`, добавлена опциональная verify-gate `SMOKE_WEB_E2E=1` в `scripts/verify-all.sh`; чекбокс Phase 4 для e2e smoke сценариев (`login/join/send/voice connect-disconnect`) переведён в `[x]`.
  - Validation: `bash -n scripts/smoke-web-e2e.sh`, `npm run web:build`, `SMOKE_API_URL=https://test.datowave.com npm run smoke:sso` — PASS.
  - Test rollout: `ssh mac-mini 'cd ~/srv/datowave && TEST_REF=origin/feature/web-header-profile-menu npm run deploy:test:smoke'` (SHA `49abe56`) — PASS (`smoke:sso`, `smoke:realtime`, `reconnectOk=true`).
...
- Pre-prod gate update: `smoke:web:e2e` успешно выполнен на сервере (`SMOKE_API_URL=https://test.datowave.com npm run smoke:web:e2e`), relay path закрыт (`callSignalRelayed=true`, `callRejectRelayed=true`, `callHangupRelayed=true`); в draft gate-record pending-пункты переведены в `PASS`, итог остаётся `NO-GO` до explicit owner sign-off и prod approval.

...
- Environment: `test` (`datowave-turn`)
...
- Container load snapshot (peak observed):
  - `datowave-turn`: CPU up to `18.63%`, RSS up to `63.7MiB`
  - `datowave-api-test`: near baseline (`~0.1-0.3% CPU`, `~92MiB` RSS)
  - `datowave-db-test` / `redis-test`: low/steady background load.
...
- Container CPU/RSS peaks observed:
  - `datowave-turn`: CPU up to `0.72%` in sampled window, RSS up to `27.26MiB`
  - `datowave-api-test`: CPU up to `25.56%`, RSS up to `141.1MiB`
  - `datowave-db-test`: CPU up to `4.24%`
  - `datowave-redis-test`: CPU up to `1.99%`

- NetIO deltas (sample window):
  - `datowave-api-test`: from `~1.01GB/2.60GB` to `~1.07GB/3.03GB` (approx `+60MB` rx, `+430MB` tx)
  - `datowave-turn`: minor change (`~260kB/236kB` -> `~278kB/254kB`) due local-generator harness path.
...
- `datowave-api-test`: avg `4.16%`, p95 `7.21%`, p99 `7.96%`, max `8.31%`.
- `datowave-turn`: avg `0.03%`, p95 `0.05%`, p99 `0.06%`, max `0.06%`.
- `datowave-db-test`: avg `1.50%`, p95 `4.92%`, p99 `5.45%`, max `5.66%`.
- `datowave-redis-test`: avg `1.31%`, p95 `2.56%`, p99 `2.79%`, max `2.85%`.

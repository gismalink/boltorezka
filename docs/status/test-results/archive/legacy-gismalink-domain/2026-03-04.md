...
- `SMOKE_API_URL=https://test.datowave.com SMOKE_WEB_BASE_URL=https://test.datowave.com SMOKE_BEARER_TOKEN=<token> npm run smoke:web:e2e`: FAIL (`[smoke:realtime] timeout: ack for call.offer`).
...
- `SMOKE_API_URL=https://test.datowave.com SMOKE_BEARER_TOKEN=<token> SMOKE_CALL_SIGNAL=1 SMOKE_RECONNECT=0 npm run smoke:realtime`: FAIL-fast с явной причиной (`second ticket from another user required`).

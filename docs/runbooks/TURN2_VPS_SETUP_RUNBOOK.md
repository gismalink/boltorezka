...
- Текущий TURN на `gismalink.art:5349` остаётся рабочим fallback.
- TURN2 поднимается на `46.149.71.86` и домене `turn2.gismalink.art`.
...
1. DNS A record: `turn2.gismalink.art -> 46.149.71.86`.
...
TURN2_DOMAIN=turn2.gismalink.art \
...
openssl s_client -connect turn2.gismalink.art:443 -servername turn2.gismalink.art -brief
```

Проверка auth/allocate без “реального” пользователя:

```bash
ssh root@46.149.71.86 'turnutils_uclient -v -S -T -u <turn2-username> -w "<turn2-password>" -p 443 -n 1 turn2.gismalink.art'
```
...
1. `turns:turn2.gismalink.art:443?transport=tcp`
2. `turns:gismalink.art:5349?transport=tcp`
...
1. убрать `turn2` из ICE JSON (оставить только текущий `gismalink.art:5349`),
2. redeploy в `test`,
3. smoke повторно.
